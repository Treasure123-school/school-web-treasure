...
export async function registerRoutes(app: Express): Server {
  ...
  app.post('/api/teacher/profile/setup', authenticateUser, authorizeRoles(ROLES.TEACHER), upload.fields([
    ...
    try {
      ...
      // Enhanced logging before profile creation
  ...
  app.get('/api/teacher/profile/me', authenticateUser, authorizeRoles(ROLES.TEACHER), async (req, res) => {
    try {
      ...
      console.log('📤 Sending profile to frontend:', completeProfile.subjects?.length || 0, 'subjects,', completeProfile.assignedClasses?.length || 0, 'classes');

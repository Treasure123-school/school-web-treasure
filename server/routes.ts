...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/exams/:examId/submit', authenticateUser, authorizeRoles(ROLES.STUDENT), logExamAccess, validateExamTimeWindow, async (req, res) => {
    ...
    try {
      ...
      // DISABLED: Do NOT sync exam score to report card
      // Per requirements: Students should see scores immediately after submission 
      // but scores should NOT be added to report cards
      // Report cards should be managed separately through admin/teacher actions
      console.log(`[SUBMIT] Exam score NOT synced to report card (disabled per requirements)`);
      const reportCardSync = { success: false, message: 'Report card sync disabled - scores not added to report card' };
